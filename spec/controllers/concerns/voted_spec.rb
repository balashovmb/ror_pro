  ...
  describe 'POST #vote_up' do
    ...
    context "Vote up for another user's question/answer" do
      ...
      it "change question/answer rating to 1" do
        expect { post :vote_up, params: { id: votable.id }, format: :json }.to change { votable.reload.rating }.by(1)
      end
    end
    ...
    context "Vote down for another user's question/answer" do
      ...
      it "change question/answer rating to -1" do
        expect { post :vote_down, params: { id: votable.id }, format: :json }.to change { votable.reload.rating }.by(-1)
      end
    end

    context "User is author of question" do
      it "vote up don't changes rating" do
        expect { post :vote_up, params: { id: users_votable.id }, format: :json }.not_to change { votable.reload.rating }
      end

      it "vote down don't changes rating" do
        expect { post :vote_down, params: { id: users_votable.id }, format: :json }.not_to change { votable.reload.rating }
      end
    end
